    ...
    stages {
        ...
        stage('deploy') {
            steps {
                ...
                sh 'docker service create --limit-memory 512M --network dev --hostname mmahu-config --no-resolve-image --replicas 1 --name mmahu-config -p 8888:8888 mmahu-main:5000/mmahu-config:1.0.$BUILD_NUMBER'
            }
        }
    }
}